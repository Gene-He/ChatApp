...
class SendMessageCmd implements IUserCmd {
     ...
     */
    public SendMessageCmd(String message) {
        String[] info = message.split(" ");
        Preconditions.checkArgument(info.length == 4 && info[0].equals("send"), "Illegal sendMessage message format: %s", message);

        this.roomId = Integer.parseInt(info[1]);
        this.receiverId = Integer.parseInt(info[2]);
        this.message = info[3];
    }
