...
class AddRoomCmd implements IUserCmd {
     ...
     */
    public AddRoomCmd(String message) {
        String[] info = message.split(" ");
        Preconditions.checkArgument(info.length == 6 && info[0].equals("create"), "Illegal create room message format: %s", message);

        this.roomName = info[1];
        ...
        this.schools = Collections.synchronizedSet(new HashSet<>(Arrays.asList(info[5].split(","))));
    }
    /**
